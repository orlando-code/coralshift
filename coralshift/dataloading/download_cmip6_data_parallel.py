...
parser.add_argument("--variable_id", default="mlotst", type=str)
commandline_args = parser.parse_args()

member_id = "r1i1p2f1"  # TODO: make this a command line input
...
for member_id in source_id_dict["member_ids"]:
    ...
    if "ocean_variable" in variable_id_dict.keys(): # this originally included "or EC-Earth3" (lower-res version)
        query["grid_label"] = "gr"
    else:
        query["grid_label"] = "gn"
    ...
    for plevel in variable_id_dict["plevels"]:
        ...
        if plevel is not None:
            # if seafloor, add to variable_id
            if plevel == -1:
                fname += "_seafloor"
            else:
                # add suffix for the pressure level in hPa to variable_id
                fname += "{:.0f}".format(plevel / 100)

        if do_crop:
            # if cropping, add spatial crop information to filename
            fname = fname + "_" + tuples_to_string(lats, lons) + ".nc"
        else:
            fname = fname + ".nc"
        # add filename to list with the level information as specifier
        fpaths[plevel] = os.path.join(download_folder, fname)

        # Intermediate tripolar grid .nc file before cdo regridding, handling potential .nc suffixes
        fpaths_tripolar[plevel] = os.path.join(download_folder, fname.split(".")[0] + '_tripolar.nc')

        if do_regrid:
            splitname = fname.split(".")[0] + "_latlon"
        ...
        for plevel in variable_id_dict["plevels"]:
            if plevel is not None:
                if plevel == -1:
                    ...
                    print("extracting {} hPa, ".format(plevel / 100), end="", flush=True)
            ...
            if do_download_ind:
                ...
                for i, result in enumerate(results):
                    ...
                    # if not caring about seafloor i.e. no levels?
                    if plevel == -1:
                        # TODO: sort out naming
                        ds = xa.open_dataset(result, decode_times=True,     # not all times easily decoded
                            # chunks={'time': '499MB'}
                            chunks={"i": 200, "j": 200}, 
                            # chunks = {"lev": "499MB"} # makes things worse
                            # chunks=None
                            ).isel(lev=slice(0,20))
                    else:
                        ds = xa.open_dataset(result, decode_times=True, 
                        chunks={'time': '499MB'})
...
# ds_copy[variable_id] = cmip6_da
                            ...
                            if seafloor_indices is None:
                                print("determining seafloor indices... ", end="", flush=True)
                                seafloor_indices = gen_seafloor_indices(ds.isel(time=0), var=variable_id)
                                seafloor_indices = np.broadcast_to(seafloor_indices, 
                                    (len(ds.time), len(ds.j), len(ds.i)))

                            print("\textracting seafloor values...", end="", flush=True)
                            cmip6_array = extract_seafloor_vals(ds[variable_id], seafloor_indices)
                            ds = ds.copy()[["time", "i", "j"]]
                            ds[variable_id] = (['time', 'j', 'i'], cmip6_array)

                        else:
                            cmip6_da = cmip6_da.sel(plev=plevel)    # this would have to be changed for models with different vertical coordinate names. Possibly in preprocessing
                ...
                #     chunks={"time": "499MB"},
                # )[variable_id]
                static_da = xa.open_dataset(results[0])[variable_id].isel(time=0).compute()
                cmip6_da = xa.open_mfdataset(results[:3], combine='by_coords', 
                    # chunks={'time': '499MB'}
                    chunks=None
                    )[variable_id]
                ...
                print('saving in order to regrid via cdo... ', end='', flush=True)
    ...
    if do_regrid:   # TODO: implement this
        ...
        for plevel in variable_id_dict['plevels']:
            ...
            if skip[plevel]:
                print('skipping this plevel due to existing file {}'.format(fpaths_latlon[plevel]), end='', flush=True)
                continue
           
            regrid_fname = fpaths_latlon[plevel] + ".nc" # check this
            # TODO: allow customisation of regridding method and resolution
            cdo.remapbil("/maps/rt582/coralshift/ec-consortium-025", input=fpaths_tripolar[plevel], output="regrid_fname")
