...
def get_data(
    ...
    # unep_wcmc shp fp
    unep_fp = (
        Path(config.gt_folder) / "unep_wcmc/01_Data/WCMC008_CoralReef2021_Py_v4_1.shp"
    )
    ...
    common_dataset.to_netcdf(
        Path(data_fp)
        / f"temp_{model_type}_{functions_creche.tuples_to_string(lats, lons)}_ds.nc"
    )
